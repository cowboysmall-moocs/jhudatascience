rankall <- function(outcome, num = "best") {

    ## Read outcome data
    outcome_data <- read.csv('data/outcome-of-care-measures.csv', colClasses = 'character')
    outcome_data[,11] <- suppressWarnings(as.numeric(outcome_data[,11]))
    outcome_data[,17] <- suppressWarnings(as.numeric(outcome_data[,17]))
    outcome_data[,23] <- suppressWarnings(as.numeric(outcome_data[,23]))

    ## Check that state and outcome are valid
    states   <- sort(unique(outcome_data$State))
    if (!state %in% states) stop("invalid state")
    
    outcomes <- c('heart attack', 'heart failure', 'pneumonia')
    if (!outcome %in% outcomes) stop("invalid outcome")

    ## For each state, find the hospital of the given rank
    ## Return a data frame with the hospital names and the
    ## (abbreviated) state name
    hospital_names <- vector()
    
    for (state in states) {
        
        state_stats <- subset(outcome_data, State==state)
        
        if (outcome == 'heart attack')  column <- 11
        if (outcome == 'heart failure') column <- 17
        if (outcome == 'pneumonia')     column <- 23
        
        state_stats <- state_stats[order(state_stats[, column], state_stats[, 2]),]
        invalid     <- is.na(state_stats[, column])
        state_stats <- state_stats[!invalid,]

        if (num == 'best') {

            hospital_names <- append(hospital_names, state_stats[1, 2])

        } else if (num == 'worst') {

            hospital_names <- append(hospital_names, state_stats[nrow(state_stats), 2])

        } else if (0 <= num && num <= nrow(state_stats)) {

            hospital_names <- append(hospital_names, state_stats[num, 2])

        } else {

            hospital_names <- append(hospital_names, NA)
        }
    }
    data.frame(hospital=hospital_names, state=states)
}
